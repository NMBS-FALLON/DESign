        ...
        public Takeoff TakeoffFromBB()
        {
            ...
            if (openFileDialog.ShowDialog() == true)
                foreach (var fileName in openFileDialog.FileNames)
                {
                    ...
                    var countMarkups =
                        from el in markUpFile.Elements("Markup")
                        where (string)el.Element("Count") != "" && !((string)el.Element("Subject") == "GIRDER" || (string)el.Element("Subject") == "JOIST")
                        select el;

                    foreach (var countMarkup in countMarkups)
                    {
                        System.Windows.Forms.MessageBox.Show(string.Format("Bluebeam Markup with label '{0}' has a count of {1} but is not a 'GIRDER' or 'JOIST', please confirm that this is not an error.", (string)countMarkup.Element("Label"), (int)countMarkup.Element("Count")));
                    }
                }
